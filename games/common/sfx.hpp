...
public:
 ...
 void volume(float value){if(device_)SDL_LockAudioDevice(device_);volume_=std::clamp(value,0.f,1.f);if(device_)SDL_UnlockAudioDevice(device_);}
 void reset(){if(device_)SDL_LockAudioDevice(device_);consumed_=0;read_=0;write_=0;for(auto& v:voices_)v.age=99;if(device_)SDL_UnlockAudioDevice(device_);}
};
}
