...
void Race::step(){
 ...
 if(!playable_&&current_.finishedAt>=0&&current_.time-current_.finishedAt>=11){
 ...
 for(int i=0;i<RacerCount;i++){
  ...
  c.wallCooldown=std::max(0.f,c.wallCooldown-dt);
  for(float* timer:{&c.shield,&c.magnet,&c.pulse,&c.stun})*timer=std::max(0.f,*timer-dt);
  const bool human=human_[i];const auto input=controls_[i];
  if(c.item!=Item::None&&go){c.itemAge+=dt;if(human?(input.tapped(arcade::West)||input.tapped(arcade::LB)):(c.itemAge>1.1f+i*.11f)){
  ...
  if(human){
   const float throttle=std::max(input.throttle,input.down(arcade::South)?1.f:0.f);
   target=throttle*30.f;
   if(input.brake>.1f||input.down(arcade::East))target=0;
   bool drift=go&&input.down(arcade::RB)&&old.speed>12;
   if(drift&&std::abs(input.x)>.18f)c.driftCharge=std::min(2.4f,c.driftCharge+dt);
   if(!drift&&old.drifting){if(c.driftCharge>.45f){c.boost=c.driftCharge>1.5f?2.3f:c.driftCharge>.9f?1.4f:.75f;++counters.boosts;cue(saver::Cue::Boost,i);}c.driftCharge=0;}
   c.drifting=drift;c.targetLane=old.lane;
  }
  if(!human&&c.decision<=0&&go){
   ...
   for(float lane:{-4.1f,0.f,4.1f}){
    ...
    for(int j=0;j<RacerCount;j++)if(i!=j&&(!playable_||before[j].finishedAt<0)){
  ...
  if(human){
   float desiredYaw=input.x*(.14f+old.speed*(c.drifting?.030f:.023f));
   c.yawSpeed=old.yawSpeed+(desiredYaw-old.yawSpeed)*std::min(1.f,dt*6);
   c.heading=clamp(old.heading+(c.yawSpeed-pose.curvature*old.speed*std::cos(old.heading))*dt,-1.05f,1.05f);
   c.laneSpeed=old.speed*std::sin(c.heading);
  }else c.laneSpeed=clamp(old.laneSpeed+laneAccel*dt,-2.6f,2.6f);
  if(!go)c.laneSpeed=0;
  c.lane=old.lane+c.laneSpeed*dt;
  // A merger yields instead of cutting through the neighboring kart.
  for(int j=0;j<RacerCount;j++)if(i!=j&&!human&&(!playable_||before[j].finishedAt<0)){
  ...
  if(human&&std::abs(c.lane)>4.65f){target*=.62f;c.laneSpeed=0;c.heading*=.80f;if(c.wallCooldown<=0&&old.speed>8){cue(saver::Cue::Hit,i);c.wallCooldown=.7f;}}
  ...
  for(int j=0;j<RacerCount;j++)if(i!=j&&(!playable_||before[j].finishedAt<0)){
   ...
   if(gap>0&&gap<22&&dl<2.9f){
    ...
    if(!human)target=std::min(target,std::max(0.f,safe));
   }
  }
  if(!go)target=0;
  if(playable_&&c.finishedAt>=0)target=0;
  if(!playable_&&current_.winner>=0)target=std::min(target,14.f);
  c.speed=std::max(0.f,old.speed+clamp(target-old.speed,-(human&&(input.brake>.1f||input.down(arcade::East))?28:16)*dt,(human?10:7)*dt));
  c.distance=old.distance+c.speed*dt*(human?std::max(.25f,std::cos(c.heading)):1.f);
  if(go&&c.finishedAt<0&&c.distance>=track.length*3)c.finishedAt=current_.roundTime-4;
  c.steer=old.steer+((human?input.x*.40f:clamp(pose.curvature*13-c.laneSpeed*.11f,-.40f,.40f))-old.steer)*dt*7;
  float desiredDrift=human?(c.drifting?-input.x*.42f:-input.x*.08f):clamp(pose.curvature*c.speed*.30f,-.26f,.26f);
 ...
 for(int pass=0;pass<4;pass++)for(int i=0;i<RacerCount;i++)for(int j=i+1;j<RacerCount;j++){
  ...
  if(playable_&&(a.finishedAt>=0||b.finishedAt>=0))continue;
 ...
 std::stable_sort(current_.order.begin(),current_.order.end(),[&](int a,int b){const auto& x=current_.cars[a];const auto& y=current_.cars[b];if(x.finishedAt>=0&&y.finishedAt>=0)return x.finishedAt<y.finishedAt;if((x.finishedAt>=0)!=(y.finishedAt>=0))return x.finishedAt>=0;return x.distance>y.distance;});
...
RaceState Race::sample()const{
 ...
 for(int i=0;i<RacerCount;i++){
  ...
  c.heading=b.heading+(n.heading-b.heading)*a;
 }
 return r;
}
}
