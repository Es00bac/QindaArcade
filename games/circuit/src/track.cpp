...
V3 Track::curve(double u)const{
 ...
 if(kind==3){
  // Authored route: depot straight, monitor wash, cableworks hairpin and wind-pump ridge.
  static const std::array<V3,14> knots{{{145,8,0},{150,12,60},{95,17,110},{18,7,108},{-45,5,68},{-120,10,105},{-156,12,58},{-130,6,3},{-85,3,-20},{-130,9,-88},{-70,14,-130},{10,17,-100},{65,10,-65},{110,6,-42}}};
  double wrapped=u-std::floor(u),position=wrapped*knots.size();int i=int(position);float t=float(position-i);
  auto p=[&](int index){return knots[(index+int(knots.size()))%knots.size()];};
  return (p(i)*2+(p(i+1)-p(i-1))*t+(p(i-1)*2-p(i)*5+p(i+1)*4-p(i+2))*(t*t)+(-p(i-1)+p(i)*3-p(i+1)*3+p(i+2))*(t*t*t))*.5f;
 }
 if(kind==1){ // A botanical valley with an elevated aqueduct on its eastern ridge.
  return {float(135*std::cos(a)+20*std::cos(3*a)),float(8+8*std::sin(a)+4*std::cos(3*a)),float(102*std::sin(a)+14*std::sin(2*a))};
 }
 if(kind==2){
  return {float(132*std::cos(a)+21*std::cos(3*a)),float(17*std::sin(2*a)+5*std::sin(3*a)),float(115*std::sin(a)+13*std::sin(3*a))};
 }
 // A city viaduct follows the block perimeter; no knot crossings or piers through the road.
 return {float(158*std::cos(a)+18*std::cos(3*a)),float(20+8*std::sin(2*a)+4*std::cos(3*a)),float(118*std::sin(a)+22*std::sin(2*a))};
}
TrackPose Track::exact(double u)const{
 // A metre-scale derivative avoids float cancellation that produced sawtooth banking.
 constexpr double e=.0005;
...
Track::Track(std::uint64_t s,int k):seed(s),kind(k){
 if(kind<0||kind>3)throw std::invalid_argument("Unknown course");
...
const char* Track::name()const{return kind==0?"AFTERGLOW VIADUCT":kind==1?"REACTOR GARDENS":kind==2?"AURORA PASS":"BLISS RECLAMATION";}
