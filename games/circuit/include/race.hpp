...
struct Car {
 ...
 float pace=25,decision=0,driftCharge=0,heading=0,yawSpeed=0,wallCooldown=0;bool drifting=false;
 int rank=1;
...
class Race {
 ...
 bool playable_=false;std::array<bool,8> human_{};std::array<arcade::Control,8> controls_{};
 friend struct RaceTest;
...
public:
 ...
 void configurePlayers(int count,const std::array<int,4>& characters){if(count<1||count>4)throw std::invalid_argument("Invalid local player count");std::array<bool,8> selected{};for(int i=0;i<count;i++){if(characters[i]<0||characters[i]>=8||selected[characters[i]])throw std::invalid_argument("Each racer must be unique");selected[characters[i]]=true;}playable_=true;human_=selected;}
 void control(int car,const arcade::Control& c){if(car>=0&&car<8)controls_[car].merge(c);}
...
struct SceneOptions {Camera camera=Camera::Director;int focus=-1;bool reduced=false;bool gallery=false;int galleryId=0;float galleryAngle=0;bool galleryMenu=false;};
void buildTrackMeshes(Frame&,const Track&);
float groundHeight(const Track&,float x,float z);
void buildLandscape(Frame&,const Track&);
void landscape(Frame&,const Track&,double);
