...
layout(location=9) in vec4 vertexTint;
layout(location=10) in vec3 vertexFinish;
...
void main(){
 ...
 metric=p*vec3(length(model[0].xyz),length(model[1].xyz),length(model[2].xyz));
 normal=normalize(transpose(inverse(mat3(model)))*n);uv=aUV;color=albedo;mat=surface;
 if(vertexTint.a>.5){color.rgb*=vertexTint.rgb;mat.xyz=vertexFinish;}
