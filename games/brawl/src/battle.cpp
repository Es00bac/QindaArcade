...
std::array<Platform,4> platforms(int stage,double time){
 ...
 if(stage==3)return {{{0,0,11.8f,3.7f,true},{-7,3.1f,2.2f,1.55f,false},{6.5f,4.2f,2.7f,1.65f,false},{-1,6.5f,2.5f,1.65f,false}}};
...
const char* stageName(int id){return id==1?"REACTOR GARDEN":id==2?"AFTERGLOW ROOFTOP":id==3?"BLISS SALVAGE YARD":"PRISM TERMINAL";}
Battle::Battle(std::uint64_t seed,int stage,int count):rng_(seed),seed_(seed),stageChoice_(stage),count_(count){
 ...
 if(stage< -1||stage>3)throw std::runtime_error("Stage out of range");
...
void Battle::configurePlayers(int count,const std::array<int,4>& characters){
 if(count<1||count>4||count>current_.active)throw std::invalid_argument("Invalid local player count");
 playable_=true;human_.fill(false);std::array<bool,8> used{};
 for(int i=0;i<count;i++){if(characters[i]<0||characters[i]>7)throw std::invalid_argument("Invalid character");human_[i]=true;current_.fighters[i].id=characters[i];used[characters[i]]=true;}
 int next=0;for(int i=count;i<current_.active;i++){while(next<8&&used[next])next++;current_.fighters[i].id=next++%8;}
 previous_=current_;
}
void Battle::player(int i){
 auto& f=current_.fighters[i];auto& c=controls_[i];
 if(f.stun>0||busy(f.action))return;
 if(std::abs(c.x)>.15f)f.face=c.x>0?1:-1;
 if(f.action==Action::Shield){if((!c.down(arcade::LB)&&!c.down(arcade::LT))||f.shield<.04f)begin(i,Action::Idle);else return;}
 if(c.tapped(arcade::South)&&f.jumps>0){f.vy=13.4f;f.floor=-1;--f.jumps;begin(i,Action::Jump);++counters.jumps;}
 if(c.y>.65f&&f.floor>0){f.drop=.3f;f.floor=-1;f.y-=.04f;f.vy=-3;}
 if(c.tapped(arcade::East)&&c.y<-.55f&&f.floor<0&&!f.recoveryUsed){f.vy=17;f.vx=c.x*7;f.recoveryUsed=true;f.invulnerable=.24f;begin(i,Action::Recovery);++counters.recoveries;return;}
 if((c.down(arcade::LB)||c.down(arcade::LT))&&f.floor>=0&&f.shield>.08f){begin(i,Action::Shield);return;}
 if(c.tapped(arcade::RB)){begin(i,Action::Dodge);f.vx=(std::abs(c.x)>.1f?c.x:float(f.face))*9.5f;return;}
 if(f.cooldown>0)return;
 if(c.tapped(arcade::RT)){begin(i,Action::Grab);++counters.grabs;}
 else if(c.tapped(arcade::West))begin(i,f.floor<0?Action::Aerial:Action::Jab);
 else if(c.tapped(arcade::North))begin(i,Action::Heavy);
 else if(c.tapped(arcade::East)&&f.specialCooldown<=0){begin(i,Action::Special);f.specialCooldown=2.4f;}
}
...
void Battle::step(){
 ...
 if(!playable_&&s.finishedAt>=0&&s.roundTime-s.finishedAt>5.5){reset(s.round+1,s.time);return;}
 ...
 for(int i=0;i<s.active;i++){
  ...
  if(!fighting){f.vx=approach(f.vx,0,dt*16);if(s.finishedAt>=0&&s.winner==i)f.action=Action::Celebrate;}
   ...
   if(human_[i])player(i);else {f.decision-=dt;if(f.decision<=0){think(i);f.decision=rng_.range(.10,.25);}}
   if(f.stun<=0&&f.action!=Action::Dodge&&f.action!=Action::Recovery){
    float goal=0;
    if(!busy(f.action)&&f.action!=Action::Shield){
     if(human_[i])goal=controls_[i].x*speed(f.id);
     else if((std::abs(f.x)>10.8f&&f.floor<0)||f.y<0)goal=(f.x<0?1.f:-1.f)*speed(f.id);
 ...
 for(auto& c:controls_)c.pressed=0;
 ...
 if(playable_){
  // Keep the stage and off-stage recovery space in view. Zoom out quickly, return gently.
  float left=-12,right=12,bottom=0,upper=8;
  for(int i=0;i<s.active;i++){const auto& f=s.fighters[i];if(f.stocks<=0||f.respawn>0)continue;left=std::min(left,f.x-1);right=std::max(right,f.x+1);bottom=std::min(bottom,f.y-1);upper=std::max(upper,f.y+3);}
  V3 focus{clamp((left+right)*.5f,-3,3),clamp((bottom+upper)*.5f,1,8),0};
  float distance=clamp(std::max((right-left)*1.05f,(upper-bottom)*2.0f),37,62);
  s.cameraTarget=mix(s.cameraTarget,focus,1-std::exp(-dt*3));
  s.cameraDistance=reach(s.cameraDistance,distance,distance>s.cameraDistance?7.f:1.2f,dt);
 }else{
