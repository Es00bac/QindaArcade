...
void initialize(Frame& f){
 ...
 for(int axis=0;axis<3;axis++)for(int side:{-1,1})append(box,parametric("",6,6,[=](float u,float v){V3 p;float *q=&p.x;q[axis]=float(side);q[(axis+1)%3]=u*2-1;q[(axis+2)%3]=v*2-1;V3 c{clamp(p.x,-.975,.975),clamp(p.y,-.975,.975),clamp(p.z,-.975,.975)};return c+unit(p-c)*.025f;},side<0));
